...
* **Required packages for MOTION:**
  ...
  * `make`
  * `cmake`
  * [`boost`](https://www.boost.org/) (version >=1.69.0)
  * `OpenMP`
...
Also, for further information see comments on the code and the doxygen documentation of MOTION in 
`your_build_folder/doc` that can be generated by adding `-DMOTION_BUILD_DOC=On` to the `cmake` command.
