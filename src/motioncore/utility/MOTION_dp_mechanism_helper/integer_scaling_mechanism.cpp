...
FLType geometric_sampling_binary_search(FLType L0, FLType R0, double lambda, std::size_t iterations,
  ...
  for (std::size_t j = 1; j < iterations; j++) {
    M_vector[j] = L_vector[j - 1] -
                  (log(0.5) + log(1 + exp(-lambda * (R_vector[j - 1] - L_vector[j - 1])))) / lambda;

    bool cond_Mj_leq_L_j_minus_1 = !(M_vector[j] > L_vector[j - 1]);

    bool cond_Mj_geq_R_j_minus_1 = !(M_vector[j] < R_vector[j - 1]);

    bool cond_Mj_gt_L_j_minus_1_lt_R_j_minus_1 =
        !(cond_Mj_leq_L_j_minus_1 | cond_Mj_geq_R_j_minus_1);

    M_vector[j] = cond_Mj_leq_L_j_minus_1 * (L_vector[j - 1] + 1) +
                  cond_Mj_geq_R_j_minus_1 * (R_vector[j - 1] - 1) +
                  cond_Mj_gt_L_j_minus_1_lt_R_j_minus_1 * M_vector[j];

    Q_vector[j] = (exp(-lambda * (M_vector[j] - L_vector[j - 1])) - 1) /
                  (exp(-lambda * (R_vector[j - 1] - L_vector[j - 1])) - 1);
...
std::vector<IntType> symmetrical_binomial_distribution(
    double constant_sqrt_n, std::vector<std::uint64_t> signed_integer_geometric_sample_vector,
    std::vector<bool> random_bits_vector, std::vector<std::uint64_t> random_unsigned_integer_vector,
  ...
  for (std::size_t iter = 0; iter < iterations; iter++) {
    ...
    double p_i = sqrtl(2 / M_PI) / constant_sqrt_n *
                 (1.0 - 0.4 * powl(logl(constant_sqrt_n) * 2, 1.5) / constant_sqrt_n) *
                 expl(-powl(((double)(i)*M_SQRT2 / constant_sqrt_n), 2));

    std::cout << " std::exp(-pow((double(i) * 2 / constant_sqrt_n), 2)): "
              << std::exp(-pow((double(i) * 2 / constant_sqrt_n), 2)) << std::endl;

    std::cout << "p_i: " << p_i << std::endl;

    bool i_in_range = ((double)(i) >= -constant_sqrt_n * sqrtl(logl(constant_sqrt_n) / 2.0)) &&
                      ((double)(i) <= constant_sqrt_n * sqrtl(logl(constant_sqrt_n) / 2.0));

    std::cout << "i_in_range: " << i_in_range << std::endl;

    bool c = random_floating_point_0_1_vector[iter] < (p_i * m * powl(2, (double)(s)) / 4);
    std::cout << "(p_i * m * powl(2, (double)(s)) / 4): " << (p_i * m * powl(2, (double)(s)) / 4)
              << std::endl;
    std::cout << "choice: " << c << std::endl;
    std::cout << "random_floating_point_0_1_vector[iter]: "
              << random_floating_point_0_1_vector[iter] << std::endl;
...
symmetrical_binomial_distribution<std::uint64_t, std::int64_t, std::allocator<std::uint64_t>>(
    double constant_sqrt_n, std::vector<std::uint64_t> signed_integer_geometric_sample_vector,
    std::vector<bool> random_bits_vector, std::vector<std::uint64_t> random_unsigned_integer_vector,
    std::vector<double> random_floating_point_0_1_vector, std::size_t iteration);

template std::vector<__uint128_t>
symmetrical_binomial_distribution<__uint128_t, __int128_t, std::allocator<__uint128_t>>(
    double constant_sqrt_n, std::vector<std::uint64_t> signed_integer_geometric_sample_vector,
    std::vector<bool> random_bits_vector, std::vector<std::uint64_t> random_unsigned_integer_vector,
    std::vector<double> random_floating_point_0_1_vector, std::size_t iteration);

std::vector<long double> symmetrical_binomial_distribution_failure_probability_estimation(
    double sqrt_n, double iteration) {
  ...
  double p_coefficient =
      sqrt(2 / M_PI) / sqrt_n * (1.0 - 0.4 * powl(logl(sqrt_n) * 2, 1.5) / sqrt_n);
...
// std::vector<double>
// integer_scaling_laplace_noise_generation(double sensitivity_l1, double epsilon, std::size_t
// num_of_simd_lap, long double failure_probability,
...
//
//     std::vector<std::uint64_t> discrete_laplace_noise_vector =
//     discrete_laplace_noise_generation<std::uint64_t, std::int64_t>(scale, failure_probability,
//                                                                                                                               num_of_elements);
//
...
std::vector<double> integer_scaling_gaussian_noise_generation(double sensitivity_l1, double sigma,
...
void test_symmetrical_binomial_distribution() {
  ...
  std::vector<IntType> symmetrical_binomial_distribution_result =
      symmetrical_binomial_distribution<IntType, IntType_int, std::allocator<IntType>>(
          sqrt_n, signed_integer_geometric_sample_vector, random_bits_vector,
          random_unsigned_integer_vector, random_floating_point_0_1_vector, iterations);
  print_u128_u_neg("symmetrical_binomial_distribution[0]: ",
                   (symmetrical_binomial_distribution_result[0]));
  print_u128_u_neg("symmetrical_binomial_distribution[1]: ",
                   (symmetrical_binomial_distribution_result[1]));
}

double SigmaForGaussian(std::int64_t l0Sensitivity, double lInfSensitivity, double epsilon,
                        double delta) {
  if (delta >= 1) {
    return 0;
  }
...
double UnitNormalCDF(double value) { return 0.5 * erfc(-value * M_SQRT1_2); }

double deltaForGaussian(double sigma, double l0Sensitivity, double lInfSensitivity,
                        double epsilon) {
  double l2Sensitivity = lInfSensitivity * sqrt(double(l0Sensitivity));
...
void symmetrical_binomial_distribution_represent_as_int64_failure_probability_estimation() {
  ...
  long double failure_probability = 0.0;
  ...
  for (std::size_t s = s_lower_bound; s < 10000; s++) {
    ...
    long double probability_l_overflow =
        INT64_MAX_double_minus_km_less_than_m * (INT64_MAX_double - (long double)(s)*m) / m;
    ...
    failure_probability = failure_probability + s_greater_or_equal_zero * probability_l_overflow *
                                                    probability_geometric_output_s;
    std::cout << std::endl;
  }

  std::cout << "failure_probability: " << failure_probability << std::endl;
  std::cout << "std::log2l(failure_probability): " << std::log2l(failure_probability) << std::endl;
  ...
  for (std::size_t s = 0; s < 10000; s++) {
    ...
    failure_probability = failure_probability + (1 - s_greater_or_equal_zero) *
                                                    probability_l_overflow *
                                                    probability_geometric_output_s;
  }
  std::cout << "failure_probability: " << failure_probability << std::endl;
  std::cout << "std::log2l(failure_probability): " << std::log2l(failure_probability) << std::endl;
