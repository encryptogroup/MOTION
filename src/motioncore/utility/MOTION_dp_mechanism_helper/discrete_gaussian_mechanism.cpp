...
std::vector<bool> Bernoulli_distribution_EXP_0_1(double gamma,
    ...
    for (std::size_t j = 1; j < iterations + 1; j++) {
        ...
        std::cout << "random_floating_point_0_1[j - 1]: " << random_floating_point_0_1[j - 1]
                  << std::endl;
...
std::vector<T> discrete_gaussian_distribution_EXP(
    ...
    for (std::size_t i = 0; i < iteration_4; i++) {
        ...
        std::cout << "random_floating_point_0_1_dgau_vector[i]: "
                  << random_floating_point_0_1_dgau_vector[i] << std::endl;
        std::cout << "exp(-(abs(Y) - sigma * sigma / t) * (abs(Y) - sigma * sigma / t) / (2 * sigma "
                     "*sigma)): "
                  << exp(-(abs(Y) - sigma * sigma / t) * (abs(Y) - sigma * sigma / t) /
                         (2 * sigma * sigma))
                  << std::endl;
...
discrete_gaussian_distribution_EXP<std::uint64_t, std::int64_t, std::allocator<std::uint64_t>>(
        double sigma, std::uint64_t numerator, std::uint64_t denominator,
        std::vector<double> random_floating_point_0_1_dlap_vector,
...
long double geometric_distribution_EXP_failure_probability_estimation(T numerator, T denominator,
    ...
    if (denominator != 1) {
        ...
        failure_probability_geometric_fast_uniform =
                1 - ((1 - expl(-1.0)) / (1 - expl(-1.0 / (long double) (denominator)))) /
        ...
        result = powl(failure_probability_geometric_fast_uniform, iteration_1) +
                 powl(failure_probability_geometric_slow, iteration_2) -
                 powl(failure_probability_geometric_fast_uniform, iteration_1) *
                 powl(failure_probability_geometric_slow, iteration_2);
        return result;
...
template long double geometric_distribution_EXP_failure_probability_estimation<std::uint64_t>(
...
GeometricDistributionEXPOptimizationStruct<T> optimize_geometric_distribution_EXP_iteration(
        T numerator_init, T denominator_init, long double total_failure_probability) {
    ...
    for (std::size_t iteration_1 = iteration_1_lower_bound; iteration_1 < iteration_1_upper_bound;
        ...
        for (std::size_t iteration_2 = 1; iteration_2 < iteration_2_upper_bound; iteration_2++) {
            ...
            if (((iteration_1) <= minimum_iteration_1) &&
                (failure_probability_estimation <= total_failure_probability)) {
                ...
                if (iteration_2 < minimum_iteration_2) {
                    minimum_iteration_2 = iteration_2;
                }

                minimum_total_iteration = iteration_1 + minimum_iteration_2;
                minimum_total_MPC_time =
                        iteration_1 * iteration_1_weight + minimum_iteration_2 * iteration_2_weight;
...
optimize_geometric_distribution_EXP_iteration<std::uint64_t>(std::uint64_t numerator,
                                                             std::uint64_t denominator,
                                                             long double total_failure_probability);

template<typename T>
GeometricDistributionEXPOptimizationStruct<T> optimize_geometric_distribution_EXP_iteration(
        long double x, long double total_failure_probability) {
    ...
    for (std::size_t i = 0; i < 4; i++) {
        ...
        numerator_denominator_relative_error_vector =
                decimalToFractionWithDenominatorFixed(x, (long double) (denominator_tmp));
        ...
        if (relative_error_tmp < relative_error) {
            result_struct_1 = optimize_geometric_distribution_EXP_iteration(
                    numerator_tmp, denominator_tmp, total_failure_probability);
    ...
    numerator_denominator_relative_error_vector =
            decimalToFractionWithDenominatorFixed(x, denominator_mod);
    T numerator_tmp = numerator_denominator_relative_error_vector[0];
    ...
    result_struct_2 = optimize_geometric_distribution_EXP_iteration(numerator_tmp, denominator_tmp,
                                                                    total_failure_probability);
...
template GeometricDistributionEXPOptimizationStruct<std::uint64_t>
optimize_geometric_distribution_EXP_iteration<std::uint64_t>(long double x,
                                                             long double total_failure_probability);
...
long double discrete_laplace_distribution_EXP_failure_probability_estimation(
    ...
    long double geometric_distribution_EXP_failure_probability_estimation_result =
            geometric_distribution_EXP_failure_probability_estimation<std::uint64_t>(
                    numerator, denominator, iteration_1, iteration_2);

    long double geometric_distribution_exp_failure_probability =
            geometric_distribution_EXP_failure_probability_estimation_result;
    long double geometric_distribution_exp_success_probability =
            1 - geometric_distribution_exp_failure_probability;

    long double geometric_distribution_exp_Y_equal_0_and_success =
            (1.0 - exp(-(long double) (numerator) / (long double) (denominator))) *
            geometric_distribution_exp_success_probability;
    ...
    long double discrete_laplace_failure_probability =
            probability_B_equal_1_and_Y_equal_0_and_geometric_distribution_exp_success +
            geometric_distribution_exp_failure_probability;

    return powl(discrete_laplace_failure_probability, iteration_3);
}

template long double discrete_laplace_distribution_EXP_failure_probability_estimation<
        std::uint64_t>(std::uint64_t numerator, std::uint64_t denominator, long double iteration_1,
                       long double iteration_2, long double iteration_3);

template<typename T>
DiscreteLaplaceDistributionOptimizationStruct<T>
optimize_discrete_laplace_distribution_EXP_iteration(long double x_lap,
                                                     long double total_failure_probability) {
    ...
    //    long double minimum_total_iteration = (iteration_1_upper_bound + iteration_2_upper_bound) *
    //    iteration_3_upper_bound + iteration_3_upper_bound;
    ...
    for (std::size_t iteration_3 = 1; iteration_3 < iteration_3_upper_bound; iteration_3++) {
        GeometricDistributionEXPOptimizationStruct<T>
                geometric_distribution_exp_optimization_result_struct =
                optimize_geometric_distribution_EXP_iteration<T>(x_geo, total_failure_probability);
        ...
        std::size_t log2_denominator =
                geometric_distribution_exp_optimization_result_struct.log2_denominator;

        long double discrete_laplace_failure_probability_estimation =
                discrete_laplace_distribution_EXP_failure_probability_estimation<T>(
        ...
        if ((((iteration_1) * iteration_3) < minimum_total_iteration) &&
            ...
            //            minimum_total_iteration = ((iteration_1 + iteration_2) * iteration_3 +
            //            iteration_3);
            ...
            result_struct.minimum_total_iteration = minimum_total_iteration;
            result_struct.minimum_total_MPC_time = minimum_total_MPC_time;
            result_struct.geometric_failure_probability_estimation =
                    geometric_failure_probability_estimation;
            result_struct.discrete_laplace_failure_probability_estimation =
                    discrete_laplace_failure_probability_estimation;
            result_struct.upscale_factor = upscale_factor;
...
optimize_discrete_laplace_distribution_EXP_iteration<std::uint64_t>(
        long double x_lap, long double total_failure_probability);
...
std::vector<long double> discrete_gaussian_distribution_EXP_failure_probability_estimation(
    ...
    long double discrete_laplace_distribution_EXP_failure_probability_estimation_result =
            discrete_laplace_distribution_EXP_failure_probability_estimation(
                    numerator, denominator, iteration_1, iteration_2, iteration_3);
    ...
    long double probability_dlap_success =
            1 - discrete_laplace_distribution_EXP_failure_probability_estimation_result;
...
discrete_gaussian_distribution_EXP_failure_probability_estimation<std::uint64_t, std::int64_t>(
        long double sigma, std::uint64_t numerator, std::uint64_t denominator, long double iteration_1,
...
optimize_discrete_gaussian_distribution_EXP_iteration(long double sigma,
                                                      long double total_failure_probability) {
    ...
    DiscreteLaplaceDistributionOptimizationStruct<T>
            ...
            optimize_discrete_laplace_distribution_EXP_iteration<T>((long double) (t),
                                                                    total_failure_probability);
    ...
    //    long double minimum_total_iteration = ((iteration_1) * iteration_3) *
    ...
    for (std::size_t iteration_4 = 1; iteration_4 < iteration_4_upper_bound; iteration_4++) {
        long double failure_probability_estimation =
                discrete_gaussian_distribution_EXP_failure_probability_estimation<T, T_int>(
                        sigma, numerator, denominator, iteration_1, iteration_2, iteration_3, iteration_4)[0];
        //        if ((((iteration_1 + iteration_2) * iteration_3 + iteration_3) * iteration_4 <
        //        minimum_total_iteration) &&
        //            (failure_probability_estimation <= total_failure_probability)) {

        if ((((iteration_1) * iteration_3) * iteration_4 <
             minimum_total_iteration) &&
            (failure_probability_estimation <= total_failure_probability)) {
            //                    if ((((iteration_1 * iteration_1_weight + iteration_2 *
            ...
            optimize_discrete_gaussian_distribution_EXP_iteration_result_struct
                    ...
                    optimize_discrete_laplace_distribution_EXP_iteration_result_struct
                            .discrete_laplace_failure_probability_estimation;
            optimize_discrete_gaussian_distribution_EXP_iteration_result_struct
                    .discrete_gaussian_failure_probability_estimation = failure_probability_estimation;
            ...
            optimize_discrete_gaussian_distribution_EXP_iteration_result_struct.t =
...
optimize_discrete_gaussian_distribution_EXP_iteration<std::uint64_t, std::int64_t>(
        long double sigma, long double total_failure_probability);
...
template<typename T, typename T_int>
std::vector<T> discrete_laplace_noise_generation(double scale, long double failure_probability,
                                                 std::size_t num_of_elements) {
    ...
    DiscreteLaplaceDistributionOptimizationStruct<T>
            optimize_discrete_laplace_distribution_EXP_iteration_result_struct =
            optimize_discrete_laplace_distribution_EXP_iteration<T>(scale, failure_probability);
    std::size_t iteration_1 =
            optimize_discrete_laplace_distribution_EXP_iteration_result_struct.iteration_geo_1;
    std::size_t iteration_2 =
            optimize_discrete_laplace_distribution_EXP_iteration_result_struct.iteration_geo_2;
...
template std::vector<std::uint64_t> discrete_laplace_noise_generation<std::uint64_t, std::int64_t>(
        double scale, long double failure_probability, std::size_t num_of_elements);

template<typename T, typename T_int>
std::vector<T> discrete_gaussian_noise_generation(double sigma, long double failure_probability,
                                                  std::size_t num_of_elements) {
    std::uint64_t t = floor(sigma) + 1;

    DiscreteGaussianDistributionOptimizationStruct<T>
            optimize_discrete_gaussian_distribution_EXP_iteration_result_struct =
            optimize_discrete_gaussian_distribution_EXP_iteration<T, T_int>(sigma,
                                                                            failure_probability);
    std::size_t iteration_1 =
            optimize_discrete_gaussian_distribution_EXP_iteration_result_struct.iteration_geo_1;
    std::size_t iteration_2 =
        ...
        if (t != 1) {
            std::vector<T> discrete_gaussian_distribution_EXP_result =
                    discrete_gaussian_distribution_EXP<T, T_int, std::allocator<T>>(
                            sigma, numerator, denominator, random_floating_point_0_1_dlap_vector,
                            random_integer_dlap_vector, bernoulli_sample_dlap_vector,
                            random_floating_point_0_1_dgau_vector, iteration_1, iteration_2, iteration_3,
                            iteration_4);
            ...
            std::vector<T> discrete_gaussian_distribution_EXP_result =
                    discrete_gaussian_distribution_EXP<T, T_int, std::allocator<T>>(
                            sigma, numerator, denominator, random_floating_point_0_1_dlap_vector,
                            bernoulli_sample_dlap_vector, random_floating_point_0_1_dgau_vector, iteration_2,
                            iteration_3, iteration_4);
            std::cout << "discrete_gaussian_distribution_EXP_result[0]: "
                      << T_int(discrete_gaussian_distribution_EXP_result[0]) << std::endl;
            //            std::cout << "discrete_gaussian_distribution_EXP_result[1]: " <<
...
template std::vector<std::uint64_t> discrete_gaussian_noise_generation<std::uint64_t, std::int64_t>(
        double sigma, long double failure_probability, std::size_t num_of_elements);
...
void test_optimize_geometric_distribution_EXP_iteration() {
    ...
    for (std::size_t i = 0; i < num_of_elements; i++) {
        ...
        result_struct = optimize_geometric_distribution_EXP_iteration<std::uint64_t>(
                scale_tmp, total_failure_probability);
        ...
        std::cout << "failure_probability_estimation: "
                  << result_struct.geometric_failure_probability_estimation << std::endl;
...
void test_optimize_discrete_laplace_distribution_EXP_iteration() {
    ...
    for (std::size_t i = 0; i < num_of_elements; i++) {
        ...
        result_struct = optimize_discrete_laplace_distribution_EXP_iteration<std::uint64_t>(
                scale_tmp, total_failure_probability);
        ...
        std::cout << "discrete_laplace_best_iterations_3: " << result_struct.iteration_dlap_3
                  << std::endl;
        std::cout << "minimum_total_iteration: " << result_struct.minimum_total_iteration << std::endl;
        std::cout << "minimum_total_MPC_time: " << result_struct.minimum_total_MPC_time << std::endl;
        ...
        std::cout << "geometric_failure_probability_estimation: "
                  << result_struct.geometric_failure_probability_estimation << std::endl;
        std::cout << "log2(geometric_failure_probability_estimation): "
                  << log2l(result_struct.geometric_failure_probability_estimation) << std::endl;

        std::cout << "numerator: " << result_struct.numerator << std::endl;
        std::cout << "denominator: " << result_struct.denominator << std::endl;

        std::cout << "discrete_laplace_failure_probability_estimation: "
                  << result_struct.discrete_laplace_failure_probability_estimation << std::endl;
        std::cout << "log2(discrete_laplace_failure_probability_estimation): "
                  << log2l(result_struct.discrete_laplace_failure_probability_estimation) << std::endl;
...
void test_discrete_gaussian_distribution_EXP_failure_estimation() {
    ...
    for (std::size_t i = 0; i < num_of_elements; i++) {
        ...
        for (std::size_t i = 1; i < 10; i++) {
            ...
            result_vector =
                    discrete_gaussian_distribution_EXP_failure_probability_estimation<std::uint64_t,
                            std::int64_t>(
                            sigma_tmp, std::uint64_t((long double) (numerator_tmp) * upscale_factor),
                            std::uint64_t((long double) (denominator_tmp) * upscale_factor), iteration_1,
                            iteration_2, iteration_3, iteration_4);

            std::cout << "discrete_laplace_failure_probability_estimation: " << result_vector[0]
                      << std::endl;
            std::cout << "================================================================" << std::endl;
        }
    }
...
void test_optimize_discrete_gaussian_distribution_EXP_iteration() {
    ...
    for (std::size_t i = 0; i < num_of_elements; i++) {
        ...
        result_struct =
                optimize_discrete_gaussian_distribution_EXP_iteration<std::uint64_t, std::int64_t>(
                        sigma_tmp, total_failure_probability);
        std::cout << "geo_best_iterations_1: " << result_struct.iteration_geo_1 << std::endl;
        std::cout << "geo_best_iterations_2: " << result_struct.iteration_geo_2 << std::endl;
        std::cout << "discrete_laplace_best_iterations_3: " << result_struct.iteration_dlap_3
                  << std::endl;
        std::cout << "discrete_gaussian_best_iterations_4: " << result_struct.iteration_dgauss_4
                  << std::endl;
        std::cout << "minimum_total_iteration: " << result_struct.minimum_total_iteration << std::endl;
        std::cout << "minimum_total_MPC_time: " << result_struct.minimum_total_MPC_time << std::endl;
        ...
        std::cout << "discrete_gaussian_failure_probability_estimation: "
                  << result_struct.discrete_gaussian_failure_probability_estimation << std::endl;
...
void test_optimize_integer_scaling_laplace_distribution_EXP_iteration() {
    ...
    // k in [10, 45]
    long double pow2_k = std::exp2l(15);


    for (std::size_t i = 0; i < num_of_elements; i++) {
        std::cout << "i: " << i << std::endl;
        long double epsilon_tmp = epsilon_double_vector[i];
        long double sensitivity_l1 = 1;

        std::cout<<"epsilon_tmp: "<<epsilon_tmp<<std::endl;

        // TODO: check parameter calculation
        long double resolution_r = ceil_power_of_two(sensitivity_l1 / epsilon_tmp / pow2_k);
        long double log2_resolution_r = std::log2(resolution_r);
        long double delta_r = sensitivity_l1 + resolution_r;
        long double lambda = resolution_r * epsilon_tmp / delta_r;
        long double scale_tmp = 1.0 / lambda;

        std::cout << "sensitivity_l1_ / epsilon / pow2_k_: " << sensitivity_l1 / epsilon_tmp / pow2_k
                  << std::endl;
        std::cout << "resolution_r: " << resolution_r << std::endl;
