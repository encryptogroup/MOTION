...
struct pool_ctx {
  pool_ctx( std::uint32_t thread_count, bool suspend)
    : thread_count_(thread_count), suspend_(suspend), counter_(0), schedulers_(thread_count, nullptr), barrier_(thread_count) {
...
std::shared_ptr<pool_ctx> pooled_work_stealing::create_pool_ctx( std::uint32_t thread_count, bool suspend) {
...
pooled_work_stealing::pooled_work_stealing( std::shared_ptr<pool_ctx> pool_ctx )
  :pool_ctx_{pool_ctx}, id_{pool_ctx_->counter_++},
...
void
pooled_work_stealing::awakened( boost::fibers::context * ctx) noexcept {
...
pooled_work_stealing::pick_next() noexcept {
...
void
pooled_work_stealing::suspend_until( std::chrono::steady_clock::time_point const& time_point) noexcept {
    if ( suspend_) {
...
void
pooled_work_stealing::notify() noexcept {
    if ( suspend_) {
        std::unique_lock< std::mutex > lk{ mtx_ };
        flag_ = true;
        lk.unlock();
