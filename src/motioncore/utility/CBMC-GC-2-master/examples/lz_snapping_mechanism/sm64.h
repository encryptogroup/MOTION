...
#define FLOATINGPOINT_EXPONENT_MASK (((FLType(1) << FLOATINGPOINT_EXPONENT_BITS) - 1) << FLOATINGPOINT_MANTISSA_BITS)
...
FLType round_to_nearest_integer_CBMC(FLType x, FLType not_used)
{
    ...
    int16_t unbiased_exponent_num_y =
        (int16_t)(x_exponent >> FLOATINGPOINT_MANTISSA_BITS) - (int16_t)(FLOATINGPOINT_EXPONENT_BIAS);
    ...
    else if (unbiased_exponent_num_y >= 0)
    {
        ...
        mantissa_fraction_msb_mask[FLOATINGPOINT_MANTISSA_BITS - 1] =
            (unbiased_exponent_num_y == (int16_t)(FLOATINGPOINT_MANTISSA_BITS));

        bool mantissa_fraction_msb =
            mantissa_fraction_msb_mask[FLOATINGPOINT_MANTISSA_BITS - 1] & mantissa_array[FLOATINGPOINT_MANTISSA_BITS - 1];
        for (i = 1; i < FLOATINGPOINT_MANTISSA_BITS; i++)
        {
            mantissa_fraction_msb_mask[FLOATINGPOINT_MANTISSA_BITS - 1 - i] =
                (unbiased_exponent_num_y == (int16_t)(FLOATINGPOINT_MANTISSA_BITS - i - 1));
        ...
        //        for (i = 0; i < FLOATINGPOINT_MANTISSA_BITS; i++) {
        //            mantissa_integer_bit[i] = ((FLType) (mantissa_integer_array[i]) << (FLOATINGPOINT_MANTISSA_BITS - 1 - i));
        //        }
