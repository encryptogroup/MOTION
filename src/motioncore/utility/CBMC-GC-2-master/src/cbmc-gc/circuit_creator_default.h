...
public:
  ...
  //----------------------------------------------------------------------------
  // Add a CNF clause
  virtual void lcnf(bvt const &bv) override { throw std::runtime_error{"circuit_creator_defaultt::lcnf() not supported"}; }
...
inline void depth_first_traversal(std::unordered_map<int, tmp_nodet> const &graph, literalt root)
{
  std::unordered_map<int, traversal_markt> marks;
}

inline void depth_first_traversal_visit(literalt cur, std::unordered_map<int, tmp_nodet> const &graph,
                           std::unordered_map<int, traversal_markt> &marks)
{
  auto &mark = marks[cur.dimacs()];
  if(mark == traversal_markt::temp)
    throw std::runtime_error{"cycle detected!"};
  
  mark = traversal_markt::temp;
  auto &node = graph.at(cur.dimacs());
  if(node.kind != tmp_node_kindt::input)
  {
    depth_first_traversal_visit(node.input_left, graph, marks);
    depth_first_traversal_visit(node.input_right, graph, marks);
  }

  mark = traversal_markt::perm;
}
                                     ...
                                     circuit_creator_defaultt const &src, simple_circuitt &circuit)
{
  ...
  gate->add_fanin(*convert_node(cur_node.input_left, literal_to_gate, src, circuit), 0);
  gate->add_fanin(*convert_node(cur_node.input_right, literal_to_gate, src, circuit), 1);
...
//==================================================================================================
inline void to_dot(circuit_creator_defaultt const &src, std::vector<mpc_variable_infot> const &vars)
{
  std::ofstream dot_file{"circuit.dot"};
  dot_file << "digraph {\n";
  ...
  int num_xor = 0;
  int num_unknown = 0;
  for(auto &pair: src.graph())
  {
    tmp_nodet const &node = pair.second;
    if(pair.second.kind == tmp_node_kindt::l_and)
    {
      num_and++;
      dot_file << "\t" << node.input_left.dimacs() << " -> " << node.me.dimacs() << ";\n";
      dot_file << "\t" << node.input_right.dimacs() << " -> " << node.me.dimacs() << ";\n";
    }
    else if(pair.second.kind == tmp_node_kindt::l_or)
    {
      num_or++;
      dot_file << "\t" << node.input_left.dimacs() << " -> " << node.me.dimacs() << ";\n";
      dot_file << "\t" << node.input_right.dimacs() << " -> " << node.me.dimacs() << ";\n";
    }
    else if(pair.second.kind == tmp_node_kindt::l_xor)
    {
      num_xor++;
      dot_file << "\t" << node.input_left.dimacs() << " -> " << node.me.dimacs() << ";\n";
      dot_file << "\t" << node.input_right.dimacs() << " -> " << node.me.dimacs() << ";\n";
    }
    else if(pair.second.kind == tmp_node_kindt::input)
    {
      num_unknown++;
      dot_file << "\t" << node.me.dimacs() << ";\n";
    }
  }
  ...
  std::cout << "num_or = " << num_or << std::endl;
  std::cout << "num_xor = " << num_xor << std::endl;
  std::cout << "num_unknown = " << num_unknown << std::endl;
  std::cout << "num_total = " << src.graph().size() << std::endl;
}
