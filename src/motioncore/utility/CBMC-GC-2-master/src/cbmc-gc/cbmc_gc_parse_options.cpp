...
// Most of the interesting stuff is happening in cbmc_gc_parse_optionst::doit().


/*******************************************************************\

Function: cbmc_gc_parse_optionst::cbmc_gc_parse_optionst
...
Function: cbmc_gc_parse_optionst::eval_verbosity
...
Function: cbmc_gc_parse_optionst::get_command_line_options
...
void cbmc_gc_parse_optionst::get_command_line_options(optionst &options)
{
  ...
  // [CBMC-GC]
  if(cmdline.isset("outdir"))
    options.set_option("outdir", cmdline.get_value("outdir"));
  else
    options.set_option("outdir", ".");
...
Function: cbmc_gc_parse_optionst::doit
...
int cbmc_gc_parse_optionst::doit()
{
  ...
  // TODO [CBMC-GC] Instanciate our implementation of prop_convt here!

  building_blockst bb = options.get_bool_option("low-depth-bb") ?
    get_lowdepth_building_blocks() : get_default_building_blocks();
  expr_optimizationt expr_opt = options.get_bool_option("low-depth-expr") ?
    expr_optimizationt::low_depth : expr_optimizationt::standard;
  ...
  prop->set_message_handler(ui_message_handler);
  bv_cbmc_gct* bv_cbmc = new bv_cbmc_gct(ns, *prop, bb, expr_opt);
  bmc_gct bmc(options, symbol_table, ui_message_handler, *bv_cbmc);
  ...
  double minimization_time_limit = -1.0; // = no limit
  if(cmdline.isset("minimization-time-limit"))
    minimization_time_limit = atof(cmdline.get_value("minimization-time-limit").c_str());
  ...
  bmc.symex().set_message_handler(get_message_handler());
  bmc.symex().options=options;
  status() << "Starting Bounded Model Checking" << eom;
  bmc.symex().last_source_location.make_nil();
  bmc.setup_unwind();
  // now run symbolic execution
  bmc.symex()(goto_functions);

  // convert to boolean formula
  // [CBMC-GC] We use convert_assignments() and not convert() because the latter also converts
  //           assertions, assumptions, guards and lots of other stuff that we don't need. We only
  //           care for what the program actually DOES, so convert_assignments() is enough.
  bmc.symex_equation().convert_assignments(*bv_cbmc);

  // [CBMC-GC] Convert boolean formula to circuit
  auto io_variables = get_input_output_variables(bv_cbmc->get_map().mapping, config.main);
  simple_circuitt circuit{*this};
  status() << "Converting to circuit..." << eom;
  prop->create_circuit(io_variables, circuit);
  
  // [CBMC-GC] Do circuit minimization
  if(do_minimization)
  {
    auto stats_before_minimization = circuit.query_stats();
    circuit.minimize(false, 0, minimization_time_limit, false);
	circuit.print_stats(stats_before_minimization);
    status() << "After minimization:" << eom;
	circuit.print_stats();
  }
  else
	circuit.print_stats();
...
Function: cbmc_gc_parse_optionst::set_properties
...
bool cbmc_gc_parse_optionst::set_properties(goto_functionst &goto_functions)
{
  try
  {
...
Function: cbmc_gc_parse_optionst::get_goto_program
...
int cbmc_gc_parse_optionst::get_goto_program(
  const optionst &options,
  bmc_gct &bmc, // for get_modules
  goto_functionst &goto_functions)
{
...
Function: cbmc_gc_parse_optionst::preprocessing
...
Function: cbmc_gc_parse_optionst::process_goto_program
...
bool cbmc_gc_parse_optionst::process_goto_program(
  const optionst &options,
  goto_functionst &goto_functions)
{
  try
...
Function: cbmc_gc_parse_optionst::help
