...
void simple_circuitt::collect_predecessors(gatet* gate) {
	...
	if (gate->fanin0 != NULL) {
		// l_v for the direct predecessor
		calculate_delay_entry(gate, gate->fanin0, (gate_delay + gate->fanin0->gate_label));

		// l_v for all indirect predecessors (i.e. G_v of the direct predecessor)
		for (gatet::predst::iterator it = gate->fanin0->preds->begin(); it != gate->fanin0->preds->end(); ++it)
			calculate_delay_entry(gate, (*it).first, ((*it).second + gate_delay));
	}
	if (gate->fanin1 != NULL) {
		calculate_delay_entry(gate, gate->fanin1, (gate_delay + gate->fanin1->gate_label));

		for (gatet::predst::iterator it = gate->fanin1->preds->begin(); it != gate->fanin1->preds->end(); ++it)
			calculate_delay_entry(gate, (*it).first, ((*it).second + gate_delay));
	}
}
...
bool simple_circuitt::can_insert(gatet* root, gatet* gate, std::set<gatet*>* inputs) {
	...
	if(gate->fanin0 != NULL) {
		...
		if (root->cluster->find(gate->fanin0) == root->cluster->end() && inputs->find(gate->fanin0) == inputs->end())
			fanins++;
	}

	if(gate->fanin1 != NULL) {
		// fanin gate is not already contained in the cluster and also not in the input set -> new input
		if (root->cluster->find(gate->fanin1) == root->cluster->end() && inputs->find(gate->fanin1) == inputs->end())
			fanins++;
	}
...
void simple_circuitt::insert_gate(gatet* root, gatet* gate, std::set<gatet*>* inputs) {
	...
	if(gate->fanin0 != NULL && root->cluster->find(gate->fanin0) == root->cluster->end())
		inputs->insert(gate->fanin0);

	if(gate->fanin1 != NULL && root->cluster->find(gate->fanin1) == root->cluster->end())
		inputs->insert(gate->fanin1);
}
...
void simple_circuitt::compute_inputs(gatet::clustert* cluster, gate_sett* set_I, gate_sett* set_inputs) {
	for(gate_sett::iterator it = cluster->begin(); it != cluster->end(); ++it) {
		...
		if((*it)->fanin0 != NULL) {
			if (cluster->find((*it)->fanin0) == cluster->end())
				set_I->insert((*it)->fanin0);

			// we also need INPUTs as LUT fanins but we do not want to build a cluster for them
			if ((*it)->fanin0->operation == INPUT)
				set_inputs->insert((*it)->fanin0);
		}

		if((*it)->fanin1 != NULL) {
			if (cluster->find((*it)->fanin1) == cluster->end())
				set_I->insert((*it)->fanin1);

			if ((*it)->fanin1->operation == INPUT)
				set_inputs->insert((*it)->fanin1);
		}
...
void simple_circuitt::replace_with_LUT(gatet* root, std::set< gatet* >* inputs, std::string* result) {
	...
	//LUT_gates->push_back(LUT_gate);
	root->replace_by(LUT_gate);
}

/*******************************************************************
 Function: simple_circuitt::do_checks

 Inputs: string with name of method that calls this function

 Outputs: void

 Purpose: does some tests on the computed LUT gates (all inputs/outputs set etc.)

 \*******************************************************************/
void simple_circuitt::do_checks(std::string caller) {
	if (LUT_gates->size() != 0) {
		std::cout << std::endl;
		std::cout << caller << ": do some checks if LUTs are ok" << std::endl;
		for (::std::vector< gatet* >::iterator it3 = LUT_gates->begin(); it3 != LUT_gates->end(); ++it3) {
			if (*it3 != NULL) {
				LUT_gatet* l_gate = dynamic_cast<LUT_gatet*> (*it3);
				assert(l_gate);
				assert(l_gate->get_fanins()->size() == LUT_SIZE);
				assert(l_gate->fanouts.size() > 0);

				for(std::set< std::pair< gatet*, unsigned > >::iterator it4 = l_gate->get_fanins()->begin(); it4 != l_gate->get_fanins()->end(); ++it4)
					assert(l_gate != (*it4).first);
			}
		}
