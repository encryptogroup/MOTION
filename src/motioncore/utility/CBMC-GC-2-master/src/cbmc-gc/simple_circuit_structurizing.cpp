...
bool simple_circuitt::structural_minimize(gatet& gate, GATE_OP op) {
	...
	while (act_gate->get_operation() == op && act_gate->fanouts.size() == 1) {
		if (act_gate->fanin0->get_operation() == op && act_gate->fanin1->get_operation() != op) {
			act_list.push_back(act_gate->fanin1);
			act_gate = act_gate->fanin0;
		}
		else if (act_gate->fanin0->get_operation() != op && act_gate->fanin1->get_operation() == op) {
			act_list.push_back(act_gate->fanin0);
			act_gate = act_gate->fanin1;
		}
		else {
			act_list.push_back(act_gate->fanin0);
			act_list.push_back(act_gate->fanin1);
