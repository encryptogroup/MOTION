...
bool simple_circuitt::convert_OR(gatet& gate, gatet*& fanin0_out, gatet*& fanin1_out) {
	if (gate.get_operation() == NOT)
		return convert_NOR(*(gate.fanin0), fanin0_out, fanin1_out);
	return false;
}
...
bool simple_circuitt::convert_XNOR2(gatet& gate, gatet*& fanin0_out, gatet*& fanin1_out) {
	...
	if (convert_NOR(gate, nor0, nor1)) {
		if(nor0->get_operation() == AND && nor1->get_operation() == AND) {
			if (nor0->fanin0->get_operation() == NOT) {
				if (nor1->fanin0->get_operation() == NOT && nor0->fanin0->fanin0 == nor1->fanin1 && nor0->fanin1 == nor1->fanin0->fanin0) {
					fanin0_out = nor0->fanin0->fanin0;
					fanin1_out = nor0->fanin1;
					return true;
				}
				else if (nor1->fanin1->get_operation() == NOT && nor0->fanin0->fanin0 == nor1->fanin0 && nor0->fanin1 == nor1->fanin1->fanin0) {
					fanin0_out = nor0->fanin0->fanin0;
					fanin1_out = nor0->fanin1;
					return true;
				}
			}
			if (nor0->fanin1->get_operation() == NOT) {
				//ToDo: might be a bug. There is a segmentation fault when used with recursive XOR if clause 2
				/*if (nor1->fanin0->get_operation() == NOT && nor0->fanin1->fanin0 == nor1->fanin1 && nor0->fanin0 == nor1->fanin0->fanin0) {
					fanin0_out = nor0->fanin1->fanin0;
					fanin1_out = nor0->fanin0;
					return true;
				}
				else*/ if (nor1->fanin1->get_operation() == NOT && nor0->fanin1->fanin0 == nor1->fanin0 && nor0->fanin0 == nor1->fanin1->fanin0) {
					fanin0_out = nor0->fanin1->fanin0;
					fanin1_out = nor0->fanin0;
					return true;
				}
...
bool simple_circuitt::convert_XNOR1(gatet& gate, gatet*& fanin0_out, gatet*& fanin1_out) {
	...
	if (gate.get_operation() == NOT)
		return convert_XOR2(*(gate.fanin0), fanin0_out, fanin1_out);
...
bool simple_circuitt::convert_XOR1(gatet& gate, gatet*& fanin0_out, gatet*& fanin1_out) {
	if (gate.get_operation() == NOT)
		return convert_XNOR2(*(gate.fanin0), fanin0_out, fanin1_out);
...
bool simple_circuitt::convert_XOR2(gatet& gate, gatet*& fanin0_out, gatet*& fanin1_out) {
	...
	if (convert_NOR(gate, nor0, nor1)) {
		...
		if (nor0->get_operation() == AND && nor1->get_operation() == AND) {
			if (nor0->fanin0->get_operation() == NOT && nor0->fanin1->get_operation() == NOT) {
				if ((nor0->fanin0->fanin0 == nor1->fanin0 && nor0->fanin1->fanin0 == nor1->fanin1)
					||(nor0->fanin0->fanin0 == nor1->fanin1 && nor0->fanin1->fanin0 == nor1->fanin0)) {
					fanin0_out = nor1->fanin0;
					fanin1_out = nor1->fanin1;
					return true;
				}
			}
			if (nor1->fanin0->get_operation() == NOT && nor1->fanin1->get_operation() == NOT) {
				if ((nor1->fanin0->fanin0 == nor0->fanin0 && nor1->fanin1->fanin0 == nor0->fanin1)
					||(nor1->fanin0->fanin0 == nor0->fanin1 && nor1->fanin1->fanin0 == nor0->fanin0)) {
					fanin0_out = nor0->fanin0;
					fanin1_out = nor0->fanin1;
					return true;
				}
			}
...
bool simple_circuitt::convert_XOR3(gatet& gate, gatet*& fanin0_out, gatet*& fanin1_out) {
	if (gate.get_operation() == NOT)
		return convert_XNOR3(*(gate.fanin0), fanin0_out, fanin1_out);

	return false;
}

bool simple_circuitt::convert_XNOR_NAND(gatet& gate, gatet*& fanin0_out, gatet*& fanin1_out) {
	if (gate.get_operation() == NOT && gate.fanin0->get_operation() == AND)

		return (fanin0_out == gate.fanin0->fanin0 && fanin1_out == gate.fanin0->fanin1) || (fanin0_out == gate.fanin0->fanin1 && fanin1_out == gate.fanin0->fanin0);
...
bool simple_circuitt::convert_XNOR3(gatet& gate, gatet*& fanin0_out, gatet*& fanin1_out) {
	...
	if (convert_NOR(gate, nor0, nor1)) {
		if (nor0->get_operation() == AND && nor1->get_operation() == AND) {
			if (convert_XNOR_NAND(*(nor0->fanin0), nor0->fanin1, nor1->fanin1) && nor0->fanin0 == nor1->fanin0) {
				fanin0_out = nor0->fanin1;
				fanin0_out = nor1->fanin1;
				return true;
			}
			if (convert_XNOR_NAND(*(nor0->fanin0), nor0->fanin1, nor1->fanin0) && nor0->fanin0 == nor1->fanin1) {
				fanin0_out = nor0->fanin1;
				fanin0_out = nor1->fanin0;
				return true;
			}
			if (convert_XNOR_NAND(*(nor0->fanin1), nor0->fanin0, nor1->fanin1) && nor0->fanin1 == nor1->fanin0) {
				fanin0_out = nor0->fanin0;
				fanin0_out = nor1->fanin1;
				return true;
			}
			if (convert_XNOR_NAND(*(nor0->fanin1), nor0->fanin0, nor1->fanin0) && nor0->fanin1 == nor1->fanin1) {
				fanin0_out = nor0->fanin0;
				fanin0_out = nor1->fanin0;
				return true;
			}
...
bool simple_circuitt::propagate_NOT(gatet& gate) {
	if (gate.get_operation() == XOR) {
		...
		if (has_this_NOT_fanout(gate.fanin0, gate.fanin1, XOR, this_gate) && gate.fanin0->get_operation() != INPUT && gate.fanin1->get_operation() != INPUT) {
			// hat unser Gate ein fanout Gate mit dem gleichen anderen fanin wie ein fanout Gate des gefundenen?
			for (gatet::fanoutst::iterator fanout_it = (*gate.fanin0).fanouts.begin(); fanout_it != (*gate.fanin0).fanouts.end(); ++fanout_it) {
				gatet::fanoutt* act_gate = *fanout_it; // aktuelles fanout Gate
				gatet* new_gate; // gleiches fanout Gate des anderen
				gatet* input1 = get_other_input(*(act_gate->first), gate.fanin0); // anderer Input des aktuellen fanout Gates
				has_this_fanout(this_gate, input1, XOR, new_gate); //finde gleiches Gate
				// gehe weiter nach unten mit new_gate und act_gate
			}
		}
		if (has_this_NOT_fanout(gate.fanin1, gate.fanin0, XOR, this_gate) && gate.fanin0->get_operation() != INPUT && gate.fanin1->get_operation() != INPUT) {
