...
bvt bv_cbmc_gct::convert_add_sub_lowdepth(const exprt &expr)
{
  ...
  std::vector<std::vector<bvt>> mults;
  for(exprt::operandst::const_iterator it=operands.begin(); it!=operands.end(); it++)
  {
    ...
    {
      std::vector<bvt> mult;
      for(exprt::operandst::const_iterator it2=(*it).operands().begin(); it2!=(*it).operands().end(); it2++)
      {
        if(it2->type()!=(*it).type())
          throw "multiplication with mixed types";

        mult.push_back(convert_bv(*it2));
  ...
  return multiplier_adder_lowdepth(prop, std::move(summands), mults);
}


bool has_negative_operand(exprt const &e)
{
  const exprt::operandst &operands=e.operands();
  for(exprt::operandst::const_iterator it=operands.begin(); it!=operands.end(); it++)
  {
    if(it->id()=="unary-" || it->id()=="no-overflow-unary-minus")
      return true;
...
bvt bv_cbmc_gct::convert_add_sub(const exprt &expr)
{
  const typet &type=ns.follow(expr.type());
  ...
  if(optimization == expr_optimizationt::low_depth)
  {
    if(operands.size() >= 3 && (type.id() == ID_unsignedbv || type.id() == ID_signedbv))
...
bvt bv_cbmc_gct::convert_unary_minus(const unary_exprt &expr)
{
  const typet &type=ns.follow(expr.type());
...
bvt bv_cbmc_gct::convert_mult(const exprt &expr)
{
...
bvt bv_cbmc_gct::convert_div(const div_exprt &expr)
{
...
bvt bv_cbmc_gct::convert_mod(const mod_exprt &expr)
{
...
bvt bv_cbmc_gct::convert_index(const index_exprt &expr)
{
...
bvt bv_cbmc_gct::convert_shift(const binary_exprt &expr)
{
...
bvt bv_cbmc_gct::convert_if(const if_exprt &expr)
{
  std::size_t width=boolbv_width(expr.type());
  ...
  return bv_if(prop, cond, op1_bv, op2_bv);
}


literalt bv_cbmc_gct::convert_equality(const equal_exprt &expr)
{
...
literalt bv_cbmc_gct::convert_bv_rel(const exprt &expr)
{
  const exprt::operandst &operands=expr.operands();
