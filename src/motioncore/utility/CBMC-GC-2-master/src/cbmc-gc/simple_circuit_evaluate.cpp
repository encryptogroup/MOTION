...
bool simple_circuitt::evaluate_rec(gatet* gate, std::map< gatet*, bool >* eval_map) {
	...
	if (gate->fanin0 != NULL) {
		std::map< gatet*, bool >::iterator map_it = eval_map->find(gate->fanin0);
		if (map_it == eval_map->end())
			fanin0 = evaluate_rec(gate->fanin0, eval_map);
		else fanin0 = (*map_it).second;
	}
	else std::cout << "no fanin0 op: " << gate->operation << std::endl;

	if (gate->fanin1 != NULL) {
		std::map< gatet*, bool >::iterator map_it = eval_map->find(gate->fanin1);
		if (map_it == eval_map->end())
			fanin1 = evaluate_rec(gate->fanin1, eval_map);
		else fanin1 = (*map_it).second;
	}
	else if (gate->operation != NOT && gate->operation != OUTPUT)
		std::cout << "no fanin1. Op: " << gate->operation << std::endl;
