...
//==================================================================================================
SymbolTable create_symbol_table(circ::Circuit const &c)
{
	SymbolTable table;
	for(auto &input: c.name_to_inputs)
		table[input.first] = {circ::Type{input.second.type}, circ::SymbolEntry::input};

	for(auto &output: c.name_to_outputs)
		table[output.first] = {circ::Type{output.second.type}, circ::SymbolEntry::output};

	return table;
}

circ::Context create_context(circ::Circuit const &circuit)
{
	// Pre-define input and output variables

	circ::Context ctx;
	for(auto &input: circuit.name_to_inputs)
		ctx.add(input.first) = circ::TypedValue{circ::Type{input.second.type}};

	for(auto &output: circuit.name_to_outputs)
		ctx.add(output.first) = circ::TypedValue{circ::Type{output.second.type}};
...
//==================================================================================================
static void simulate_gate(Circuit const &circ, GateID id, std::unordered_map<ElementID, bool> &values)
{
	...
	switch(gate.kind)
	{
		...
		case GateKind::and_gate:
		{
			...
			values[id] = values[in_a] & values[in_b];
		} break;
...
static void simulate_element(Circuit const &circ, std::unordered_map<ElementID, bool> &values, ElementID id)
{
	if(id.kind() == ElementID::Kind::gate)
		simulate_gate(circ, id.as_gate_id(), values);
	else if(id.kind() == ElementID::Kind::output)
		values[id] = values.at(*circ.outputs[id.id()]);
}

static void set_input_value(
	InputVariable const &var, TypedValue const &value,
	std::unordered_map<ElementID, bool> &circuit_values)
{
	...
	{
		circuit_values[var.inputs[idx]] = bit;
	});
}
...
{
	std::unordered_map<ElementID, bool> values{
		{const_one_id(), true}
	};
