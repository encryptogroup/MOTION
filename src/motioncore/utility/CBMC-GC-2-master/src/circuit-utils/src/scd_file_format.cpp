...
public:
		...
		m_has_const{false}
	{
		...
		topological_traversal(circuit, [&](ElementID id)
		{
			...
			else if(id.kind() == ElementID::Kind::const_one)
				m_has_const = true;
		});
	}
	...
	uint64_t operator () (ElementID id) const
	{
		switch(id.kind())
		{
			...
			case ElementID::Kind::gate: return m_gate_to_wire[id.id()] + m_has_const;
			case ElementID::Kind::output:
			{
				ElementID fanin = m_circuit.outputs[id.id()].value();
				return (*this)(fanin);
			}
			case ElementID::Kind::const_one: return m_num_inputs + 1;
		}
	}

	std::vector<ElementID> const& sorted_gates() const { return m_sorted_gates; }
	bool has_const_wire() const { return m_has_const; }
	uint64_t total_num_objects() const
	{
		return 3 + 3 * (m_sorted_gates.size() + m_has_const) + m_circuit.outputs.size();
	}
...
void write_scd_circuit(Circuit const &circuit, std::ostream &file)
{
	...
	{
		Gate const &gate = circuit[sorted_gates[i].as_gate_id()];
		assert(gate.num_fanins == 1 || gate.num_fanins == 2);

		if(gate.num_fanins == 1)
			file << MsgPackUInt{dummy_wire};
		else
			file << MsgPackUInt{id_to_wire(gate.fanins[1])};
	}

	if(id_to_wire.has_const_wire())
		file << MsgPackUInt{0b1111};
	...
	for(auto const &opt_fanin: circuit.outputs)
		file << MsgPackUInt{id_to_wire(opt_fanin.value())};
}
