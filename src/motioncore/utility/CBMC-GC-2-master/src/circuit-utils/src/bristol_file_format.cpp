...
GateKind read_gate_kind(std::ifstream &file)
{
	std::string word;
	file >> word;
	...
	throw std::runtime_error{"Unsupported gate type: " + word};
}
...
public:
	...
	void operator () (ElementID from, ElementID to)
	{
		...
		// this our circuit structure, so we need to handle this case specially.
		...
		if(from.kind() == ElementID::Kind::output)
		{
			...
			if(m_circuit[oid])
				m_circuit.add_wire(*m_circuit[oid], to);
			else
				m_to_be_wired.push_back({oid, to});
		}
		else
			m_circuit.add_wire(from, to);
	}

	void add_remaining_wires()
	{
		for(auto const &wire: m_to_be_wired)
			m_circuit.add_wire(m_circuit[wire.first].value(), wire.second);
	}
...
Circuit read_bristol_circuit(std::ifstream &file)
{
	...
	std::fill_n(std::back_inserter(circuit.inputs), num_input_a_wires, Party::alice);
	std::fill_n(std::back_inserter(circuit.inputs), num_input_b_wires, Party::bob);
	...
	for(uint64_t i = 0; i < num_gates; ++i)
	{
		...
		if(num_inputs == 1)
		{
			...
			circuit[gate_id].kind = GateKind::not_gate;
...
std::pair<uint64_t, uint64_t> get_num_inputs(Circuit const &circuit)
{
	...
	for(Party party: circuit.inputs)
	{
...
bool is_const_gate_used_by_normal_gates(Circuit const &circuit)
{
	...
	for(Gate const &gate: circuit.gates)
	{
		...
		{
			if(gate.fanins[i].kind() == ElementID::Kind::const_one)
				return true;
		}
	}
...
uint64_t count_const_gates_used_by_outputs(Circuit const &circuit)
{
	...
	for(optional<ElementID> const &fanin: circuit.outputs)
		count += fanin.value().kind() == ElementID::Kind::const_one;
...
uint64_t count_additional_gates_for_outputs(Circuit const &circuit)
{
	...
	{
		bool inserted = unique_fanins.insert(opt_fanin.value()).second;
		if(!inserted) // duplicate
		{
			// The additional gates required for a constant gate are already counted in
			// count_const_gates().
			if(opt_fanin.value().kind() != ElementID::Kind::const_one)
				additional_gates += 1;
		}
	}
...
public:
		...
		m_const_one_wire(-1)
	{
		...
		m_num_gates = circuit.gates.size() +
		...
		{
			ElementID fanin = circuit.outputs[i].value();
			...
			if(fanin.kind() == ElementID::Kind::gate && m_gate_to_wire[fanin.id()] == (uint64_t)-1)
	...
	uint64_t operator() (ElementID id)
	{
		switch(id.kind())
		{
			...
			case ElementID::Kind::const_one:
				if(m_const_one_wire == (uint64_t)-1)
					m_const_one_wire = m_wire_counter++;

				return m_const_one_wire;
...
void write_gate(IDToWireConverter &id_to_wire, Gate const &gate, uint64_t fanout, std::ostream &file)
{
	if(gate.kind == GateKind::not_gate)
		file << "1 1 " << id_to_wire(gate.fanins[0]) << ' ' << fanout << " INV\n";
	else
	{
		file << "2 1 " << id_to_wire(gate.fanins[0]) << ' ' << id_to_wire(gate.fanins[1])
			 << ' ' << fanout
...
void write_bristol_circuit(Circuit const &circuit, std::ofstream &file)
{
	...
	topological_traversal(circuit, [&](ElementID id)
	{
		...
		else if(id.kind() == ElementID::Kind::const_one && write_const_gate)
		{
			write_constant(circuit, id_to_wire, id_to_wire(const_one_id()), file);
	...
	for(auto wire: id_to_wire.additional_output_wires())
	{
		...
		else if(from.kind() == ElementID::Kind::const_one)
