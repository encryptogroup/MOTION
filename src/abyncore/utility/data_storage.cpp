...
void DataStorage::SetReceivedOutputMessage(std::vector<u8> &&output_message) {
  ...
  auto output_message_ptr =
      ABYN::Communication::GetOutputMessage(message->payload()->data());

  auto gate_id = output_message_ptr->gate_id();

  auto ret =
      received_output_messages_.insert({gate_id, std::move(output_message)});
  if (!ret.second) {
    logger_->LogError(
  ...
  logger_->LogDebug(fmt::format(
      "Received an output message from Party#{} for gate#{}", id_, gate_id));
}

const ABYN::Communication::OutputMessage *DataStorage::GetOutputMessage(
    std::size_t gate_id) {
  ...
  return ABYN::Communication::GetOutputMessage(
      output_message->payload()->data());
}

const ABYN::Communication::HelloMessage *
DataStorage::GetReceivedHelloMessage() {
  if (received_hello_message_.empty()) {
    return nullptr;
  }
  auto hello_message =
      ABYN::Communication::GetMessage(received_hello_message_.data());
