...
Configuration::Configuration(
    const std::vector<CommunicationContextPtr> &contexts, std::size_t id)
    : my_id_(id) {
  ...
  for (auto &p : contexts) {
    if (p->GetId() < 0) {
      throw(std::runtime_error(fmt::format(
          "IDs should be positive integers, found: {}", p->GetId())));
      ...
      throw(std::runtime_error(
          "You cannot have multiple parties with the same id"));
    }
