...
void AESRandomnessGenerator::Initialize(u8 key[AES_KEY_SIZE],
                                        u8 iv[AES_BLOCK_SIZE / 2]) {
  ...
  std::copy(boolean_key.data(), boolean_key.data() + AES_KEY_SIZE,
            raw_key_boolean_);

  initialized_ = true;
};

std::vector<u8> AESRandomnessGenerator::GetBits(std::size_t gate_id,
  ...
  std::copy(std::begin(aes_ctr_nonce_boolean_),
            std::end(aes_ctr_nonce_boolean_), input.data());
  while (random_bits_counter < (gate_id + num_of_gates)) {
    auto counter_pointer =
        input.data() + AESRandomnessGenerator::COUNTER_OFFSET;
    ...
    *reinterpret_cast<u64 *>(counter_pointer) = counter_value;
    ...
    int output_length =
        Encrypt(ctx_boolean_.get(), input.data(), output.data(), 1);
    ...
    random_bits.insert(random_bits.end(), output.begin(),
                       output.begin() + AES_BLOCK_SIZE);
...
int AESRandomnessGenerator::Encrypt(evp_cipher_ctx_st *ctx, u8 *input,
                                    u8 *output, std::size_t num_of_blocks) {
  int output_length, len;

  if (1 != EVP_EncryptInit_ex(ctx, EVP_aes_128_ecb(), NULL, raw_key_arithmetic_,
                              nullptr)) {
    throw(
        std::runtime_error(fmt::format("Could not re-initialize EVP context")));
  }

  if (1 != EVP_EncryptUpdate(ctx, output, &len, input,
                             AES_BLOCK_SIZE * num_of_blocks)) {
  ...
  if (1 != EVP_EncryptFinal_ex(ctx, output + len, &len)) {
    throw(std::runtime_error(
        fmt::format("Could not finalize EVP-AES encryption")));
  }
...
std::vector<u8> AESRandomnessGenerator::HashKey(
    const u8 old_key[AES_KEY_SIZE]) {
...
std::vector<u8> AESRandomnessGenerator::GetSeed() {
  std::vector<u8> seed(raw_key_arithmetic_,
