...
class AESRandomnessGenerator {
 public:
  AESRandomnessGenerator(std::size_t party_id)
      : party_id_(party_id),
        ctx_arithmetic_(MakeCipherCtx()),
        ctx_boolean_(MakeCipherCtx()) {
    if (!ctx_arithmetic_ || !ctx_boolean_) {
      throw(
          std::runtime_error(fmt::format("Could not initialize EVP context")));
    }
  }

  void Initialize(unsigned char key[AES_KEY_SIZE],
  ...
  T GetUnsigned(std::size_t gate_id) {
    ...
    std::copy(std::begin(aes_ctr_nonce_arithmetic_),
              std::end(aes_ctr_nonce_arithmetic_), input);
  ...
  std::vector<T> GetUnsigned(std::size_t gate_id, std::size_t num_of_gates) {
    ...
    std::vector<u8> output(size_in_bytes + AES_BLOCK_SIZE),
    ...
    for (auto i = 0u; i < num_of_gates; ++i, ++gate_id_copy) {
      ...
      std::copy(reinterpret_cast<u8 *>(&gate_id_copy),
                ...
                input.data() + i * AES_BLOCK_SIZE +
                    AESRandomnessGenerator::COUNTER_OFFSET);
    }

    // encrypt as in CTR mode, but without sequentially incrementing the counter
    // after each encryption
    int output_length = Encrypt(ctx_arithmetic_.get(), input.data(),
                                output.data(), num_of_gates);
    assert(output_length >= 0);

    if (static_cast<std::size_t>(output_length) < size_in_bytes ||
        static_cast<std::size_t>(output_length) >
            size_in_bytes + AES_BLOCK_SIZE) {
      throw(std::runtime_error(
          fmt::format("AES encryption output has length {}, expected {}",
    ...
    for (auto i = 0u; i < num_of_gates; ++i) {
      ...
      single_result <<= 64;
      single_result ^=
          reinterpret_cast<u64 *>(output.data())[i * 2 + 1] ^ gate_id++;
      single_result %= mod;
      results.push_back(static_cast<T>(
          single_result));  // static-cast the result to the smaller ring
    }
 ...
 private:
  ...
  u8 aes_ctr_nonce_arithmetic_[AES_BLOCK_SIZE /
                               2] = {0},
  ...
  /// each encryption.
  ///
  int Encrypt(evp_cipher_ctx_st *ctx, u8 *input, u8 *output,
