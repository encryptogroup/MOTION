...
#pragma omp taskloop num_tasks(n) default(shared)
    for (auto destination_id = 0u; destination_id < n; ++destination_id) {
      ...
      backend_->GetLogger()->LogDebug(fmt::format(
...
std::vector<std::unique_ptr<Party>> Party::GetNLocalParties(
    std::size_t num_parties, u16 port) {
  if (num_parties < 3) {
    throw(std::runtime_error(
        fmt::format("Can generate only >= 3 local parties, current input: {}",
                    num_parties)));
  }
  ...
  auto portid = [](u32 my_id, u32 other_id) -> u32 {
    return other_id < my_id ? (other_id << 16) + (my_id)
                            : (my_id << 16) + (other_id);
  };
...
#pragma omp taskloop num_tasks(num_parties)
  for (auto my_id = 0ul; my_id < num_parties; ++my_id) {
    ...
    for (auto other_id = 0ul; other_id < num_parties; ++other_id) {
      if (my_id == other_id) continue;
      ABYN::Role role =
          other_id < my_id ? ABYN::Role::Client : ABYN::Role::Server;
      ...
      if (search != assigned_ports.end()) {
        ...
        throw(std::runtime_error(fmt::format(
            "Didn't find the port id in the lookup table: {}", port_id)));
      };
