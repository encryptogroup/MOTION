...
namespace ABYN {
  ...
  ABYNBackend::ABYNBackend(ABYNConfigurationPtr &abyn_config) : abyn_config_(abyn_config) {
    ...
    for (auto i = 0u; i < abyn_config_->GetNumOfParties(); ++i) {
      randomness_generator_.push_back(std::make_unique<RandomnessGenerator>(i, this));
    }
  };
  ...
  void ABYNBackend::RandomnessGenerator::Initialize(u8 key[AES_KEY_SIZE], u8 iv[AES_BLOCK_SIZE / 2]) {
  ...
  T ABYNBackend::RandomnessGenerator::GetUnsigned(size_t gate_id) {
    ...
    constexpr auto ring_limit = RandomnessGenerator::GetRingLimit<T>();
