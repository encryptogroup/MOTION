...
//
    ...
    public:
      ArithmeticInputGate(const std::vector<T> &input, std::size_t input_owner, const ABYN::CorePtr &core) :
          input_(input) {
      ...
      ArithmeticInputGate(std::vector<T> &&input, std::size_t input_owner, const ABYN::CorePtr &core) :
      ...
      void InitializationHelper() {
        ...
        auto gate_info = fmt::format("uint{}_t type, gate id {}, owner {}", sizeof(T) * 8, gate_id_, input_owner_);
      ...
      void Evaluate() final {
        ...
        if (static_cast<std::size_t>(input_owner_) == my_id) {
          ...
          auto &rand_generator = core_->GetConfig()->
              GetCommunicationContext(static_cast<std::size_t>(input_owner_))->GetTheirRandomnessGenerator();
    ...
    public:
      ...
      //perhaps, we should return a copy of the pointer and not move it for the case we need it multiple times
      ABYN::Shares::ArithmeticSharePtr<T> GetOutputAsArithmeticShare() {
        auto arithmetic_wire = std::dynamic_pointer_cast<ABYN::Wires::ArithmeticWire<T>>(output_wires_.at(0));
      ...
      ArithmeticOutputGate(const ABYN::Wires::ArithmeticWirePtr<T> &parent, std::size_t output_owner) :
        ...
        auto gate_info = fmt::format("uint{}_t type, gate id {}, owner {}", sizeof(T) * 8, gate_id_, output_owner_);
      ...
      void Evaluate() final {
        ...
        if (is_my_output_) {
          ...
          auto arithmetic_output_wire = std::dynamic_pointer_cast<ABYN::Wires::ArithmeticWire<T>>(output_wires_.at(0));
    ...
    public:
      ...
      ArithmeticAdditionGate(const ABYN::Wires::ArithmeticWirePtr<T> &a,
        ...
        auto gate_info = fmt::format("uint{}_t type, gate id {}, parents: {}, {}", sizeof(T) * 8, gate_id_,
                                     parent_a_.at(0)->GetWireId(),
                                     parent_b_.at(0)->GetWireId());
      ...
      void Evaluate() final {
        ...
        for (auto i = 0ull; i < result.size(); ++i) {
          if (party_id_ == my_id) {
            ...
            auto &rand_generator = core_->GetConfig()->GetCommunicationContext(
                party_id_)->GetTheirRandomnessGenerator();
            Helpers::WaitFor(rand_generator->IsInitialized());
            SetSetupIsReady();
