...
CommunicationHandler::CommunicationHandler(ABYN::CommunicationContextPtr &party,
  ...
  handler_info_ = fmt::format(
      "Party#{} handler with end ip {}, local port {}, remote port {}",
      party->GetId(), party->GetIp(),
      party->GetSocket()->local_endpoint().port(),
      party->GetSocket()->remote_endpoint().port());

  sender_thread_ =
      std::thread([&]() { CommunicationHandler::ActAsSender(this); });

  receiver_thread_ =
      std::thread([&]() { CommunicationHandler::ActAsReceiver(this); });
}
...
void CommunicationHandler::SendMessage(
  ...
  if (GetMessage(message_raw_pointer)->message_type() ==
      MessageType_HelloMessage) {
    party_->GetDataStorage().SetSentHelloMessage(message_raw_pointer,
                                                 message_size);
  }
  buffer.insert(buffer.end(), message_raw_pointer,
                message_raw_pointer + message_size);
  {
    std::scoped_lock lock(queue_send_mutex_);
    queue_send_.push(std::move(buffer));
  }

  logger_->LogTrace(fmt::format("{}: Have put a {}-byte message to send queue",
                                handler_info_, message_size));
}
...
void CommunicationHandler::TerminateCommunication() {
  ...
  logger_->LogTrace(fmt::format(
      "{}: Put a termination message message to send queue", handler_info_));
}
...
void CommunicationHandler::ActAsSender(CommunicationHandler *handler) {
  while (handler->ContinueCommunication()) {
    if (!handler->GetSendQueue().empty()) {
      ...
      std::vector<u8> message_size_buffer(message.data(),
                                          message.data() + sizeof(u32));
...
#pragma omp task
    while (handler->ContinueCommunication()) {
      ...
      static_assert(sizeof(size) ==
                    MESSAGE_SIZE_BYTELEN);  // check consistency of the bytelen
      ...
      std::vector<u8> message_buffer =
          CommunicationHandler::ParseBody(handler, size);
      ...
      handler->GetLogger()->LogTrace(
          fmt::format("{}: Read message body of size {}, message: {}",
...
#pragma omp task
    while (handler->ContinueCommunication() ||
           !handler->GetReceiveQueue().empty()) {
      if (!handler->GetReceiveQueue().empty()) {
        std::vector<u8> message_buffer(
            std::move(handler->GetReceiveQueue().front()));
...
u32 CommunicationHandler::ParseHeader(CommunicationHandler *handler) {
  ...
  boost::asio::read(
      *handler->GetSocket().get(), boost::asio::buffer(message_size_buffer),
  ...
  if (size > 0) {
    if (size == TERMINATION_MESSAGE) {
      ...
      handler->GetLogger()->LogTrace(fmt::format(
          "{}: Got a termination message from the socket", handler->GetInfo()));
    } else {
      std::string s;
      for (auto i = 0u; i < 4; ++i) {
  ...
  } else if (size == 0 || (ec == boost::asio::error::would_block ||
                           ec == boost::asio::error::eof)) {
    std::this_thread::sleep_for(std::chrono::milliseconds(1));
    return 0;
  } else if (ec) {
    throw(std::runtime_error(
        fmt::format("Error while reading from socket: {}", ec.message())));
  }

  return size;
}

std::vector<u8> CommunicationHandler::ParseBody(CommunicationHandler *handler,
                                                u32 size) {
  boost::system::error_code ec;
  handler->GetSocket()->non_blocking(false);
  std::vector<u8> message_buffer(size);
  // get the message
  boost::asio::read(*(handler->GetSocket().get()),
    ...
    if (my_hm->online_after_setup() != my_hm->online_after_setup()) {
      logger_->LogError(
          fmt::format("{}: different \"online after setup\" setting: my info "
                      "is #{}, but received #{}",
                      handler_info_, my_hm->online_after_setup(),
                      their_hm->online_after_setup()));
      result = false;
    }
    if (my_hm->input_sharing_seed() == nullptr ||
        my_hm->input_sharing_seed()->size() == 0) {
